...
def get_cookie(cookie, name):
    ...
    if zone_in_str:
        zones_in_list = [
            np.frombuffer(bytes.fromhex(data), dtype=np.int64).reshape(-1, 2)
            for data in zone_in_str.split(",")
        ]
...
def generate_heatmap(image, boxes):
    ...
    for box in boxes:
        ...
        for i in range(-3, 4):
            for j in range(-3, 4):
                if (
                    0 <= center_y + i < heatmap.shape[0]
                    and 0 <= center_x + j < heatmap.shape[1]
                ):
