...
def app():
    ...
    if video is not None and img is not None and tfrom is not None and start:
        ...
        with torch.no_grad():
            with col1:
                model = Model(
                    r"C:\Users\NITRO 5\Downloads\best (3).pt",
                    conf=confidence,
                    device="cuda",
                )
                speed_processor = SpeedProcessor(tfrom)
                video = VideoAndSpeedProcessor(
                    r"{}".format(video), model, speed_processor
                )
                video.process()
                speed_average_bar_chart = Chart.generate_bar(
                    video.total_cars_speed_average, "speed"
                )
                conf_average_bar_chart = Chart.generate_bar(
                    video.confidence_averages, "conf"
                )
                cv2.imwrite(video.heatmap_path, speed_processor.satilite_image)
                pde_admin_db.insert_one(
                    {
                        "heat_map": video.heatmap_path,
                        "video_path": video.source_video_path,
                        "conf_average": conf_average_bar_chart,
                    }
                )
                speeding_vehicles = speeding_cars_db.find(
                    {"output_video": cookie.get("video")}
                )
                num_speeding_vehicles = speeding_cars_db.count_documents(
                    {"output_video": cookie.get("video")}
                )
        ...
        if speeding_vehicles:
            speeding_vehicles_list = list(
                speeding_cars_db.find({"output_video": cookie.get("video")})
            )
                ...
                for speeding_vehicle in speeding_vehicles_list[i : i + batch_size]:
                    try:
                        col = (
                            col1
                            if batch_index % 3 == 0
                            else col2 if batch_index % 3 == 1 else col3
                        )
                        with col:
