...
class VideoProcessor:
    ...
    def __init__(self, source_video_path,model):
        self.model = model
        self.trace_annotator = sv.TraceAnnotator(position = sv.Position.CENTER,trace_length=20,thickness=2)
        self.box_annotator = sv.BoxAnnotator(thickness = 2,text_thickness=1)
        self.source_video_path = source_video_path
        self.output_video_path = r"result_videos\users\{}".format(os.path.basename(source_video_path))
    ...
    def process(self):
                ...
                if success:
                    ...
                    for _,_,conf,class_id,tracker_id in detections:
                        if tracker_id not in self.tracked_objects:
                            ...
                            self.confidence_averages[self.classes_names[class_id]]+=conf
                        labels.append(f"{self.classes_dict[class_id]} {tracker_id}")
                    frame = self.box_annotator.annotate(scene=image,detections=detections,labels=labels)
                    frame = self.trace_annotator.annotate(frame,detections=detections)
                    if self.writer is None:
                        self.writer = cv2.VideoWriter(self.output_video_path,self.fourcc,vid_cap.get(cv2.CAP_PROP_FPS),(frame.shape[1],frame.shape[0]))
        ...
        for i in self.classes_names:
            self.confidence_averages[i]=(self.confidence_averages[i]/self.classes_count[i]) if self.classes_count[i] !=0 else 0 
...
class VideoAndSpeedProcessor(VideoProcessor):
    ...
    def __init__(self, source_video_path, model,speed_processor):
        ...
        self.output_video_path = r"D:\AA\the jounie project\result_videos\speed_predictions\{}".format(os.path.basename(source_video_path))
        self.heatmap_path = r"D:\AA\the jounie project\charts\heat\{}.jpg".format(os.path.basename(source_video_path))                                                                      
    ...
    def process(self):
        ...
        try:
            ...
            while(vid_cap.isOpened()):
                success,image = vid_cap.read()
                if success:
                    detections = self.model.detect_and_track(self.histogram_equalizer(image))
                    labels = list()
                    for box,_,conf,class_id,tracker_id in detections:
                        ...
                        if tracker_id not in self.tracked_objects:
                            ...
                            self.classes_count[self.classes_names[class_id]]+=1
                            self.confidence_averages[self.classes_names[class_id]]+=conf
                        self.object_postions[int(tracker_id)].append((center_x,center_y))
                        new_speed =  self.speed_processor.process(image,box,self.object_postions,self.each_car_speed_average,
                                                                  self.total_cars_speed_average,tracker_id,class_id,self.classes_names)
            
                        labels.append(f"{self.classes_dict[class_id]} {int(new_speed)}km/h")
                    frame = self.box_annotator.annotate(scene=image,detections=detections,labels=labels)
                    frame = self.trace_annotator.annotate(frame,detections=detections)
                    if self.writer is None:
                        self.writer = cv2.VideoWriter(self.output_video_path,self.fourcc,vid_cap.get(cv2.CAP_PROP_FPS),(frame.shape[1],frame.shape[0]))
        ...
        for i in self.total_cars_speed_average:
            ...
            self.total_cars_speed_average[i]  = int(sum(values)/len(values)) if len(values) != 0 else 0 
...
class SpeedProcessor:
    ...
    def process(self,image,box,object_postions,each_car_average_speed,total_cars_average_speed,tracker_id,class_id,classes_names):
        ...
        if n>1:
            ...
            p1 = self.tform.inverse((int(object_postions[tracker_id][1][0]/3), int(object_postions[tracker_id][1][1]/2)))
            p2 = self.tform.inverse((int(object_postions[tracker_id][0][0]/3) , int(object_postions[tracker_id][0][1]/2)))
            x1 , y1 = (p1[0][0] , p1[0][1])
            x2 , y2 = (p2[0][0] , p2[0][1])
            self.satilite_image[int(y2-50),int(x2)] = (0,0,255)
            speed = math.sqrt( ((x2 - x1)*self.scale_w)**2 + ((y2 - y1)*self.scale_h)**2) * self.fps * self.ppm
        ...
        if len(each_car_average_speed[tracker_id]) == 30:
            ...
            if new_speed > 74 and new_speed < 100 and tracker_id not in self.speeding_cars:
                x1, y1, x2 ,y2 = box
