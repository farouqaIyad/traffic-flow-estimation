    ...
    while (vid_cap.isOpened()):
        ...
        if success:
            if writer is None:
                ...
                writer = cv2.VideoWriter(vid_name,fourcc,  fps , (image.shape[1],image.shape[0]))

            frame = cv2.resize(image,(image.shape[1],image.shape[0]),fx=0,fy=0, interpolation = cv2.INTER_CUBIC)
            writer.write(frame)
        else:
            vid_cap.release()
            break
...
def app():
    ...
    with col1:
        ...
        if cookie.get('tunnel_video') is not None:
            ...
            video_file = st.file_uploader(label='Upload video', type=['mp4', 'avi','mov'] , key='vid')
            if video_file:
