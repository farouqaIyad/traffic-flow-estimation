    ...
    if image is None:
        ...
        if canvas_result.json_data is not None:
            ...
            if len(left)==8:
                ...
                for i in range(0,8):
                    if i <=3:
                        ZONE_IN_POLYGONS.append(np.array([[left[i],top[i]],[right[i],top[i]],[right[i],bottom[i]],[left[i],bottom[i]]]))
                    else:
                        ZONE_OUT_POLYGONS.append(np.array([[left[i],top[i]],[right[i],top[i]],[right[i],bottom[i]],[left[i],bottom[i]]]))
